# -*- coding: utf-8 -*-
# Version Library V3.3.3  - 2024-03-28 12:03
#******************************************************************************
...
#//-----------------------------------------------------------------------------
#// 2023-11-24  ZaerleM                
#//             mxAutmation3.3.3 generated on the basis of C++ lib V3.3.3
#//
#******************************************************************************/
#// Date        Programmer        Reviewer
#//             Description
#//-----------------------------------------------------------------------------
#// 13.01.2025  ElHeni, Firas                
...
class MXA_COMMAND:
    def __init__(self):
        ...
        self.BUFFERMODE = 0
        self.COMMANDSIZE = 0
        self.CMDPARREAL = [0.0] * 101
        self.CMDPARINT = [0] * 51
        self.CMDPARBOOL = [False] * 51
...
class MXA_COMMAND_STATUS:
    def __init__(self):
        self.CMDIDRET = 0
        self.ORDERIDRET = 0
        self.TRANSMISSIONNORET = 0
        self.STATERETURN = MXA_CMD_SR_ARR
        self.CMDDATARETURN = MXA_CMD_DATA_RET
        self.CMDDATARETCSKRC = 0
        self.CMDDATARETCSPLC = 0


class MXA_AUTEXT_CONTROL:
    def __init__(self):
        self.PGNO = 0
        self.PGNO_PARITY = False
        self.PGNO_VALID = False
        self.EXT_START = False
        self.MOVE_ENABLE = False
        self.MOVE_DISABLE = False
        self.CONF_MESS = False
        self.DRIVESON = False
        self.DRIVESOFF = False
...
class MXA_AUTEXT_STATE:
    def __init__(self):
        ...
        self.PERI_RDY = False
        self.ROB_CAL = False
        self.IO_ACTCONF = False
        self.STOPMESS = False
        self.PGNO_FBIT_REFL = False
        self.INTNOTAUS = False
        self.PRO_ACT = False
        self.PGNO_REQ = False
        self.APPL_RUN = False
        self.PRO_MOVE = False
        self.IN_HOME = False
        self.ON_PATH = False
        self.NEAR_POSRET = False
        self.ROB_STOPPED = False
        self.T1 = False
        self.T2 = False
        self.AUT = False
        self.EXT = False
        self.KCP_CONNECT = False
        self.BRAKETEST_MONTIME = False
        self.BRAKETEST_REQ_INT = False
        self.BRAKETEST_WORK = False
        self.BRAKES_OK = False
        self.BRAKETEST_WARN = False
        self.MASTERINGTEST_REQ_INT = False
        self.MASTERINGTESTSWITCH_OK = False
...
class MXA_KRC_STATE:
    def __init__(self):
        ...
        self.BASEACT = 0
        self.IPOMODEACT = 0
        self.POSACTVALID = False
        self.AXISACT = E6AXIS()
        self.AXISACTVALID = False
        self.OVPROACT = 0
        self.VELACT = 0.0
        ...
        self.ERRORIDSUB = 0
        self.ERRORIDPCOS = 0
        self.ACTIVEPOSORDERID = 0
        self.ACTIVEORDERIDB = 0
        self.HEARTBEAT = 0
        self.HEARTBEATPCOS = 0
        self.INTERRUPTSTATE = [0] * 9
        self.QUEUECOUNT = 0
        self.TOUCHUP = False
        self.TOUCHUP_INDEX = 0
        self.IN_VAL_1TO8 = 0
...
class MXA_JOGADVANCED:
    def __init__(self):
        self.JOG_AD_ACTIVE = False
        self.JOG_AD_STATE_VAL = 0
...
class MXA_KRC_CONTROL:
    def __init__(self):
        ...
        self.OUT_VAL_1 = False
        self.OUT_VAL_2 = False
        self.OUT_VAL_3 = False
        self.OUT_VAL_4 = False
        self.OUT_VAL_5 = False
        self.OUT_VAL_6 = False
        self.OUT_VAL_7 = False
        self.OUT_VAL_8 = False
        self.WRITE_OUT_1TO8 = False
...
class AXIS_GROUP_REF:
    def __init__(self):
        ...
        self.DEF_VEL_CP = 0.0
        self.DEF_ACC_CP = 0.0
        self.COMMAND = MXA_COMMAND()
        self.CMDSTATE = MXA_COMMAND_STATUS()
        self.AUTEXTCONTROL = MXA_AUTEXT_CONTROL()
        self.AUTEXTSTATE = MXA_AUTEXT_STATE()
        self.KRCSTATE = MXA_KRC_STATE()
        self.KRCCONTROL = MXA_KRC_CONTROL()
        self.JOG_ADVANCED = MXA_JOGADVANCED()
...
# AXIS_VEL is a list of integers
AXIS_VEL = []

# BOOL_ARRAY_40 is a list of booleans
BOOL_ARRAY_40 = []
...
# INT32_ARRAY_40 is a list of integers
INT32_ARRAY_40 = []
...
MXA_ORI_TYPE = int
MXA_PTP_APO = int
# MXA_SYNC_IO is a list of bool
MXA_SYNC_IO = []


class POSITION1:
    def __init__(self):
        self.COORDSYS_1 = COORDSYS()
        self.E6POS_1 = E6POS()
        self.E6AXIS_1 = E6AXIS()


POSITION_ARRAY = [POSITION1() for _ in range(100)]
# REAL_ARRAY_40 is a list of float
REAL_ARRAY_40 = []
REAL_ARRAY_12 = []
...
def MXA_GETIO_REAL(BYTEPOS, KRC4_INPUT):
    if (major, minor, micro) <= (2, 7, 3):
            # Convert the slice to a string, which is what struct.unpack expects in Python 2.7.3
            slice_ = str(KRC4_INPUT[BYTEPOS:BYTEPOS+4])
            O = struct.unpack('f', slice_)[0]
    else:
...
#teil 4
            ...
            if self.MOVETYPE in [0, 10, 9, 19]:
                for i in range(1, 7):
                    KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.CMDPARREAL[i] = getattr(self.AXISPOSITION, 'A%d' % i)
                for i in range(7, 13):
                    KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.CMDPARREAL[i] = getattr(self.AXISPOSITION, 'E%d' % (i-6))
            else:
                for i in range(1, 7):
                    KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.CMDPARREAL[i] = getattr(self.ACTPOSITION, ['X', 'Y', 'Z', 'A', 'B', 'C'][i-1])
                for i in range(7, 13):
                    KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.CMDPARREAL[i] = getattr(self.ACTPOSITION, 'E%d' % (i-6))
            ...
            if self.MOVETYPE in [7, 8, 17, 18]:
                for i in range(15, 27):
                    KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.CMDPARREAL[i] = getattr(self.CIRCHP, ['X', 'Y', 'Z', 'A', 'B', 'C'][i-15] if i < 21 else 'E%d' % (i-20))
...
class KRC_READAXISGROUP:
    ...
    def OnCycle(self):
        ...
        self.TON_1.IN = self.ENABLETONSUBMIT or self.ENABLETONPCOS
        self.TON_1.PT = self.HEARTBEATTO
        self.TON_1.OnCycle()
        self.M_HEARTBEATLAST = KRC_AXISGROUPREFARR[self.AXISGROUPIDX].KRCSTATE.HEARTBEAT
        self.M_HEARTBEATLASTPCOS = KRC_AXISGROUPREFARR[self.AXISGROUPIDX].KRCSTATE.HEARTBEATPCOS
        if self.TON_1._Q:
        ...
        if KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.ORDERID == self.M_ORDERIDRET:
            ...
            self.NCHECKSUM = 0
            for I in range(1, 13):
                FTMP = KRC_AXISGROUPREFARR[self.AXISGROUPIDX].CMDSTATE.CMDDATARETURN[I]
                if FTMP >= 0:
                    if FTMP - int(FTMP) >= 0.49999999:
                        NTMP1 = int(FTMP) + 1
                    else:
                        NTMP1 = int(FTMP)
                else:
                    if int(FTMP) - FTMP >= 0.49999999:
                        NTMP1 = int(FTMP) - 1
                    else:
                        NTMP1 = int(FTMP)
...
class KRC_WRITEAXISGROUP:
    ...
    def OnCycle(self):
        ...
        if self.LASTORDERID != KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.ORDERID:
            ...
            if KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.COMMANDID > 0:
                ...
                for I in range(1, 46):
                    ...
                    FTMP = KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.CMDPARREAL[I]
                    if FTMP >= 0:
                        if FTMP - int(FTMP) >= 0.49999999:
                            NTMP1 = int(FTMP) + 1
                        else:
        ...
        if KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.COMMANDID > 0:
            ...
            self.BOOLVALUES1 = 0
            for i in range(1, 16):
                globals()['self.CMDPAR_INT%d_VAL'%i] = 0
            for i in range(1, 16):
                globals()['self.CMDPAR_REAL%d_VAL'%i] = 0.0
...
# part 3
            ...
            if self.MOVETYPE == 1 or self.MOVETYPE == 3:
                    if self.M_INCREMENTACTIVE:
...
class KRC_CONVINIOFF:
    ...
    AxisGroupIdx (int): Index of axis group. Range: 1-5.
    ExecuteCmd (bool): Starts/buffers the motion in the case of a rising edge of the signal.
    ConveyorNumber (int): Number of the conveyor. Range: 1-3.
    BufferMode (int): Mode in which the statement is executed. Options: 1 (ABORTING), 2 (BUFFERED).
    ...
    def OnCycle(self):
        ...
        if self.MXA_EXECUTECOMMAND_1.WRITECMDPAR:
            KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.CMDPARINT[1] = int(self.CONVEYORNUMBER)
...
class KRC_CONVON:
    """
    The function block KRC_ConvOn activates the AMI, i.e. sets it to the #ACTIVE state. If the AMI is activated, the synchronization signals at the input of interface X33 (Fast Measurement) are evaluated. The conveyor offset can be detected out in the background leaving the robot controller free to perform other tasks. This allows the robot to carry out on-the-fly tracking of a part on the conveyor.
...
class KRC_CONVFOLLOW:
    """
    The function block KRC_ConvFollow enables the robot to follow a workpiece on the conveyor. KRC_ConvFollow can be used to define a range on the conveyor in which the robot starts to track the workpiece. If the workpiece has already exceeded the maximum conveyor distance (input MaxDistance) when the function block is called, the output MaxDistanceReached is set. This function block can only be executed if the AMI has been activated using KRC_ConvOn.

    Parameters:
    AxisGroupIdx (int): Index of axis group. Range: 1-5.
    ExecuteCmd (bool): Starts/buffers the motion in the case of a rising edge of the signal.
    ConveyorNumber (int): Number of the conveyor. Range: 1-3.
    StartDistance (float): Distance traveled by the workpiece while the robot waits before starting to track the workpiece on the conveyor. In the case of a linear conveyor: Specification in millimeters. In the case of a circular conveyor: Specification in degrees.
    MaxDistance (float): Maximum distance traveled by the workpiece before the robot starts to synchronize itself with the workpiece. In the case of a linear conveyor: Specification in millimeters. In the case of a circular conveyor: Specification in degrees.
    BufferMode (int): Mode in which the statement is executed. Options: 1 (ABORTING), 2 (BUFFERED).
    ...
    def __init__(self):
        ...
        self._DONE = False
        self._ABORTED = False
        self._MAXDISTANCEREACHED = False
        self._ERROR = False
        self._ERRORID = 0
    ...
    @property
    def MAXDISTANCEREACHED(self):
        return self._MAXDISTANCEREACHED
    ...
    def OnCycle(self):
        ...
        if self.MXA_EXECUTECOMMAND_1.WRITECMDPAR:
            KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.CMDPARINT[1] = int(self.CONVEYORNUMBER)
            KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.CMDPARREAL[1] = self.STARTDISTANCE
            KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.CMDPARREAL[2] = self.MAXDISTANCE

        self._BUSY = self.MXA_EXECUTECOMMAND_1.BUSY
        self._ACTIVE = self.MXA_EXECUTECOMMAND_1.ACTIVE
        self._DONE = self.MXA_EXECUTECOMMAND_1.DONE
        self._ABORTED = self.MXA_EXECUTECOMMAND_1.ABORTED
        self._MAXDISTANCEREACHED = self.MXA_EXECUTECOMMAND_1.CMDSPECIFIC1
        self._ERRORID = self.MXA_EXECUTECOMMAND_1.ERRORID
        self._ERROR = self._ERRORID != 0
...
class KRC_CONVSKIP:
    ...
    def OnCycle(self):
        ...
        if self.MXA_EXECUTECOMMAND_1.WRITECMDPAR:
            KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.CMDPARINT[1] = int(self.CONVEYORNUMBER)
            KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.CMDPARINT[2] = int(self.PIECENUMBER)
            KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.CMDPARREAL[1] = self.STARTDISTANCE
            KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.CMDPARREAL[2] = self.MAXDISTANCE
...
class KRC_CONVDELWPS:
    ...
    ExecuteCmd (bool): Starts/buffers the motion in the case of a rising edge of the signal.
    ConveyorNumber (int): Number of the conveyor. Range: 1-3.
    PieceNumber (int): Number of workpieces to be deleted.
...
class KRC_ACTIVATECONVINTERRUPT:
    """
    The function block KRC_ActivateConvInterrupt activates the following interrupts: Alarm distance monitoring, Maximum distance monitoring, $STOPMESS error monitoring. An interrupt cannot be processed until the interrupt has been activated by the main run of the robot interpreter. The monitoring functions are activated by the function blocks KRC_ConvFollow and KRC_ConvSkip insofar as these have been successfully synchronized with a workpiece. Calling this function block is only necessary if the monitoring function is to be ended and reactivated.
    ...
    def OnCycle(self):
        ...
        self.MXA_EXECUTECOMMAND_1.AXISGROUPIDX = self.AXISGROUPIDX
        self.MXA_EXECUTECOMMAND_1.EXECUTE = self.EXECUTECMD
        self.MXA_EXECUTECOMMAND_1.CMDID = 46
        self.MXA_EXECUTECOMMAND_1.BUFFERMODE = self.BUFFERMODE
        self.MXA_EXECUTECOMMAND_1.COMMANDSIZE = 1
        self.MXA_EXECUTECOMMAND_1.ENABLEDIRECTEXE = False
        self.MXA_EXECUTECOMMAND_1.ENABLEQUEUEEXE = True
...
class KRC_DEACTIVATECONVINTERRUPT:
    """
    The function block KRC_DeactivateConvInterrupt deactivates the following interrupts: Alarm distance monitoring, Maximum distance monitoring, $STOPMESS error monitoring. It is advisable to call this function block if leaving the conveyor area, or if monitoring is not desired.
    ...
    def OnCycle(self):
        ...
        self.MXA_EXECUTECOMMAND_1.AXISGROUPIDX = self.AXISGROUPIDX
        self.MXA_EXECUTECOMMAND_1.EXECUTE = self.EXECUTECMD
        self.MXA_EXECUTECOMMAND_1.CMDID = 47
        self.MXA_EXECUTECOMMAND_1.BUFFERMODE = self.BUFFERMODE
        self.MXA_EXECUTECOMMAND_1.COMMANDSIZE = 1
        self.MXA_EXECUTECOMMAND_1.ENABLEDIRECTEXE = False
        self.MXA_EXECUTECOMMAND_1.ENABLEQUEUEEXE = True
...
class KRC_LDDCONFIG:
    ...
    def OnCycle(self):
        ...
        if self.MXA_EXECUTECOMMAND_1.WRITECMDPAR:
            ...
            KRC_AXISGROUPREFARR[self.AXISGROUPIDX].COMMAND.CMDPARBOOL[1] = self.WARMUP
...
#part 2
        ...
        if self.NSTATE == 1:
            self.NERR = int(self.ERR_STATUS)
            if self.NERR == -1:  # Check of LDD Installation (LDD in EXT) failed (Err=-1)!
                self._ERRORID = 550
            elif self.NERR == -2:  # Check of LDD Installation (LDD in EXT) failed (Err=-2)!
                self._ERRORID = 551
            elif self.NERR == -3:  # Check of LDD Installation (LDD in EXT) failed (Err=-3)!
                self._ERRORID = 552
            elif self.NERR == -40:  # Default settings not correctly finished (Err=-40)!
                self._ERRORID = 553
            elif self.NERR == -41:  # Default settings not correctly finished (Err=-41)!
                self._ERRORID = 554
            elif self.NERR == -42:  # Default settings not correctly finished (Err=-42)!
                self._ERRORID = 555
            elif self.NERR == -45:  # A4 < 0.0 ... Move A4 to 0.0
                self._ERRORID = 556
            elif self.NERR == -46:  # A4 > 0.0 ... Move A4 to 0.0
                self._ERRORID = 557
            elif self.NERR == -102:  # Axis ranges: A2 / A3 not valid!
                self._ERRORID = 558
            elif self.NERR == -103:  # Axis ranges: A3 range not valid!
                self._ERRORID = 559
            elif self.NERR == -104:  # Axis position: A4 position not valid!
                self._ERRORID = 560
            elif self.NERR == -105:  # Axis ranges: A5 range not valid!
                self._ERRORID = 561
            elif self.NERR == -106:  # Axis ranges: A6 range not valid!
                self._ERRORID = 562
            elif self.NERR == -111:  # Axis ranges: A1 too close to the lower limit!
                self._ERRORID = 563
            elif self.NERR == -112:  # Axis ranges: A2 too close to the lower limit!
                self._ERRORID = 564
            elif self.NERR == -113:  # Axis ranges: A3 too close to the lower limit!
                self._ERRORID = 565
            elif self.NERR == -115:  # Axis ranges: A5 too close to the lower limit!
                self._ERRORID = 566
            elif self.NERR == -116:  # Axis ranges: A6 too close to the lower limit!
                self._ERRORID = 567
            elif self.NERR == -121:  # Axis ranges: A1 too close to the upper limit!
                self._ERRORID = 568
            elif self.NERR == -122:  # Axis ranges: A2 too close to the upper limit!
                self._ERRORID = 569
            elif self.NERR == -123:  # Axis ranges: A3 too close to the upper limit!
                self._ERRORID = 570
            elif self.NERR == -125:  # Axis ranges: A5 too close to the upper limit!
                self._ERRORID = 571
            elif self.NERR == -126:  # Axis ranges: A6 range not valid!
                self._ERRORID = 572
            else:
                self._ERRORID = 0
...
class KRC_LDDcheckPos:
    """
    The function block KRC_LDDcheckPos is used to check the start position for load data determination. If the start position is not suitable, an error number is generated.
    ...
    def OnCycle(self):
        ...
        self.MXA_EXECUTECOMMAND_1.AXISGROUPIDX = self.AXISGROUPIDX
        self.MXA_EXECUTECOMMAND_1.EXECUTE = self.EXECUTECMD
        self.MXA_EXECUTECOMMAND_1.CMDID = 61
        self.MXA_EXECUTECOMMAND_1.BUFFERMODE = 0
        self.MXA_EXECUTECOMMAND_1.COMMANDSIZE = 1
        self.MXA_EXECUTECOMMAND_1.ENABLEDIRECTEXE = True
        self.MXA_EXECUTECOMMAND_1.ENABLEQUEUEEXE = False
...
#part 2
        ...
        if self.NSTATE == 2:
            self.NERR = int(self.ERR_STATUS)
            if self.NERR == -4:  # LDD configuration not valid!
                self._ERRORID = 574
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            elif self.NERR == -7:  # LDD KRL Test run Program not correctly finished!
                self._ERRORID = 585
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            else:
                self._ERRORID = 0
...
#part 2
        ...
        if self.NSTATE == 2:
            ...
            if self.NERR == -1:  # Check of LDD Installation (LDD in EXT) failed (Err=-1)!
                ...
                self._ERRORID = 552
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            elif self.NERR == -4:  # LDD configuration not valid!
                self._ERRORID = 574
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            elif self.NERR == -5:  # LDD KRL Program not correctly finished!
                self._ERRORID = 575
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            elif self.NERR == -6:  # Kuka.Load 5 result unknown!
                self._ERRORID = 576
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            elif self.NERR == -40:  # Default settings not correctly finished (Err=-40)!
                self._ERRORID = 553
                ...
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
    #part 3
            if self.NERR == -115:  # Axis ranges: A5 too close to the lower limit!
                self._ERRORID = 566
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            elif self.NERR == -116:  # Axis ranges: A6 too close to the lower limit!
                self._ERRORID = 567
                ...
                self._ERRORID = 572
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            elif self.NERR == 1:  # LDD_LOAD_RESULT = #STATOVL
                self._ERRORID = 577
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            elif self.NERR == 2:  # LDD_LOAD_RESULT = #DYNOVL
                self._ERRORID = 578
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            elif self.NERR == 3:  # LDD_LOAD_RESULT = #OVL
                self._ERRORID = 579
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            elif self.NERR == 4:  # LDD_LOAD_RESULT = #SLOOR
                self._ERRORID = 580
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            elif self.NERR == 5:  # Tool Number=-1: $config.dat unchanged!
                self._ERRORID = 581
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            elif self.NERR == -11:  # LDD_start not correctly finished!
                self._ERRORID = 582
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            elif self.NERR == -12:  # Kuka.Load 5 not correctly finished!
                self._ERRORID = 583
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            elif self.NERR == -13:  # Tool number not correct!
                self._ERRORID = 584
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            elif self.NERR == -10:  # Internal Error
                self._ERRORID = 587
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            elif self.NERR == -20:  # Internal Error
                self._ERRORID = 588
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            else:
                self._ERRORID = 0
...
class KRC_LDDWRITELOAD:
    ...
    def OnCycle(self):
        ...
        if self.NSTATE == 2:
            self.NERR = int(self.ERR_STATUS)
            if self.NERR == 5:  # Tool Number=-1: $config.dat unchanged!
                self._ERRORID = 581
                KRC_AXISGROUPREFARR[self.AXISGROUPIDX].INTFBERRORID = self.ERRORID
            elif self.NERR == -11:  # LDD_start not correctly finished!
                self._ERRORID = 582
